...
def aggr_gradients(args):
        ...
        URLS = list(set([input_list[site]["URL"] for site in input_list]))
        gradients = []
        ...
        if current_iteration<iterations:
            computation_output = {"output": {"gradients":aggregated_grad,"computation_phase": 'remote',"last_iteration":False,"URL":URLS}}
        else:
            computation_output = {"output": {"gradients":aggregated_grad,"computation_phase": 'remote',"last_iteration":True,"URL":URLS}}
        return computation_output

def end_remote(args):
    input_list = args["input"]
    URLS = list(set([input_list[site]["URL"] for site in input_list]))
    computation_output = {"output": {"training_completed":True,"computation_phase": "remote","URL":URLS},"success":True}
    return computation_output
